...
#Read Files----------
seasonCompact <- fread("/Users/jamesledoux/Documents/Sports-Econometrics/TermPaper/NCAAMData/RegularSeasonCompactResults.csv")
teams <- fread(file.path("/Users/jamesledoux/Documents/Sports-Econometrics/TermPaper/NCAAMData/Teams.csv"))
...
allSeasons <- seq(1985, 2015)    #make this larger once you know it works

#get end of regular season ELO by team
...
matches2Predict <- lapply(sampleSubmission$Id, function(submissionIds){
  submissionIds <- sampleSubmission$Id[1]   #this was originally commented out
...
eloMatrix <- apply(matches2PredictDt, 1, function(matchInformation){
  ...
  eloTeam1 <- as.numeric(seasonMatrix[seasonMatrix$Player == team1, seasonMatrix$Rating])   #getting errors here
  eloTeam2 <- as.numeric(seasonMatrix[seasonMatrix$Player == team2, seasonMatrix$Rating])
  return(eloTeam1 - eloTeam2)
})
#seasonMatrix[Player == team1, Rating]
...
require(plyr)
df <- ldply(eloEndOfSeasonList, data.frame)
df <- merge(df, teams, by.x = "Player", by.y = "Team_Id")

write.csv(df, "/Users/jamesledoux/Documents/Sports-Econometrics/TermPaper/EloRatings.csv", row.names = FALSE)
