...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        if sys.argv[2] == 'directed':
            isDirected = True
        f= open('PageRankResultsTennisWomen.csv','w')
        for filename in os.listdir(directory):   #apply to every file in directory
            if not filename.startswith('.'):   #ignore hidden files/.DS_STORE
                ...
                for tup in sorted_r:
                    ...
                    f.write(str(tup[0]) + ',' + str(tup[1]) + ',' + str(yr) + '\n') 
                yr = yr + 1
        f.close() 
